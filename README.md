...
REPO_NAME = "activity-generator"
REMOTE_URL = f"https://{USERNAME}:{TOKEN}@github.com/{USERNAME}/{REPO_NAME}.git"

# Directory for the repo
LOCAL_REPO_DIR = "activity_repo"

# Create or initialize the repo
...
# Generate commits for the past month
start_date = datetime.now() - timedelta(days=30)
for i in range(30):
    commit_date = start_date + timedelta(days=i)
    file_path = os.path.join(LOCAL_REPO_DIR, f"file_{i}.txt")
    
    # Create or update file
    with open(file_path, "w") as f:
        f.write(f"Update on {commit_date.strftime('%Y-%m-%d')}")
